...
// document.getElementById("rect2").setAttribute("height","50px");
...
const suivant = () => {
    ...
    pix.src = "jpg/medium/" + pixName + ".jpg";
    buttons = keyboard.querySelectorAll("button");
    toutBoutons();
    aideDelayed();
};
...
function printWinners() {
    // if (winner){winner.parentNode.removeChild(winner)};
    // winner.parentNode.removeChild(winner);

    // winner.parentNode.removeChild(pWinner)
...
const listePix = ["ananas", "cactus", "colibri", "crocodile", "domino", "lavabo", "limace", "malade", "maracas", "radio"]
// const listePix = ["radio", "domino"];
// const listePix = ["CROCODILE"];
...
const reset = () => {
    if (tropDeFautes == false) {
        ...
        tropDeFautes = false; //reset de tropDeFautes
        suivant();
        rejouer.classList.remove("flash");
        clearAide();
        aideDelayed();
...
function toutBoutons() {
    for (let i = 0; i < buttons.length; i++) {
        function buttonsOnclick() {
            ...
            if (prenom.value.length === pixName.length) {
                // si inferieur à 4 simple smileys à la place des touches
                ...
                if (jeu1.victoires < listePix.length - 1) {
                    if (sansAide == true) {
                        ...
                        tAide = 9000;
                        console.log("mot écrit avec aide !!")
                        setTimeout(suivant, 2000);
                    }
                }
